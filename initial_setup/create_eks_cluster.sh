...
function usage {
    ...
    echo ""
    echo "usage: create_eks_cluster.sh --project string --private_subnets string"
    echo ""
...
## Functions
function set_cluster_role {
    CLUSTER_ROLE_ARN=$(aws iam get-role --role-name "$EKS_ENVIRONMENT-$project-cluster-role" --query 'Role.Arn' --output text)
}
...
function create_cluster_role {
    ...
    aws iam attach-role-policy --policy-arn arn:aws:iam::aws:policy/AmazonEKSClusterPolicy --role-name $EKS_ENVIRONMENT-$project-cluster-role 
